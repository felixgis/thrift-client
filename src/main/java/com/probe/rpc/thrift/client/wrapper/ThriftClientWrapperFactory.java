package com.probe.rpc.thrift.client.wrapper;
...
public final class ThriftClientWrapperFactory {
	...
	public static ThriftClientWrapper wrapper(final String thriftServiceId, String thriftServiceName,
			Object thriftClient, double version) {
		...
		Class<?> ifaceClass = Arrays.stream(clientClass.getDeclaringClass().getDeclaredClasses())
				.filter(clazz -> clazz.getName().endsWith("$Iface")).findFirst()
		...
		final String signature = String.join("$",
				new String[] { thriftServiceId, ifaceClass.getCanonicalName(), String.valueOf(version) });
