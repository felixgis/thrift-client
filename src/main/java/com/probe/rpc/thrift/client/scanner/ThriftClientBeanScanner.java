...
import com.probe.rpc.thrift.client.annotation.ThriftClient;
import com.probe.rpc.thrift.client.common.ThriftClientDefinitionProperty;
...
import com.probe.rpc.thrift.client.exception.ThriftClientInstantiateException;
...
public final class ThriftClientBeanScanner extends ClassPathBeanDefinitionScanner {
	...
	protected Set<BeanDefinitionHolder> doScan(String... basePackages) {
		...
		for (BeanDefinitionHolder definitionHolder : definitionHolders) {
			...
			logger.info("Scanned and found thrift client, bean {} assigned from {}", definitionHolder.getBeanName(),
			...
			String beanName = StringUtils.isNotBlank(thriftClient.value()) ? thriftClient.value()
					: (StringUtils.isNotBlank(thriftClient.name()) ? thriftClient.name()
			...
			definition.getPropertyValues().addPropertyValue(ThriftClientDefinitionProperty.BEAN_CLASS_NAME,
					beanClass.getName());
			...
			ThriftServiceSignature serviceSignature = new ThriftServiceSignature(thriftClient.serviceId(),
					thriftClient.refer(), thriftClient.version());
			...
			definition.getPropertyValues().addPropertyValue(ThriftClientDefinitionProperty.SERVICE_SIGNATURE,
					serviceSignature);
			definition.getPropertyValues().addPropertyValue(ThriftClientDefinitionProperty.CLIENT_CLASS, clientClass);
			definition.getPropertyValues().addPropertyValue(ThriftClientDefinitionProperty.CLIENT_CONSTRUCTOR,
					constructor);
	...
	private Class<? extends TServiceClient> getClientClassFromAnnotation(Class<?> beanClass) {
		...
		if (Objects.isNull(clientAwareType)) {
			throw new ThriftClientConfigException(
					"Interface annotated with @ThriftClient should be inherited from ThriftClientAware");
		}
