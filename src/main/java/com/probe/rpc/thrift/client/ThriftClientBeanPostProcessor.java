...
import com.probe.rpc.thrift.client.annotation.ThriftRefer;
import com.probe.rpc.thrift.client.common.ThriftClientAware;
	...
	public Object postProcessBeforeInitialization(Object bean, String beanName) throws BeansException {
		...
		ReflectionUtils.doWithFields(targetClass, field -> {
			...
			if (StringUtils.isNotBlank(referName)) {
				...
				injectedBean = Optional.ofNullable(injectedBean).orElseThrow(() -> new ThriftClientInstantiateException(
						"Detected non-qualified bean with name {}" + referName));
				...
				if (MapUtils.isEmpty(injectedBeanMap)) {
					throw new ThriftClientInstantiateException(
							"Detected non-qualified bean of {}" + fieldType.getSimpleName());
				}

				if (injectedBeanMap.size() > 1) {
					throw new ThriftClientInstantiateException(
							"Detected ambiguous beans of {}" + fieldType.getSimpleName());
				}

				injectedBean = injectedBeanMap.entrySet().stream().findFirst().map(Map.Entry::getValue)
			...
			if (logger.isDebugEnabled()) {
				logger.debug("Bean {} is injected into target bean {}, field {}", injectedBean, targetBean,
						field.getName());
			}
		...
		ReflectionUtils.MethodFilter methodFilter = method -> {
			...
			return Arrays.stream(method.getParameters()).map(Parameter::getType)
					.map(ThriftClientAware.class::isAssignableFrom).reduce((param1, param2) -> param1 && param2).get();
		};
		...
		ReflectionUtils.doWithMethods(targetClass, method -> {
			...
			Object objectArray = Arrays.stream(parameters).map(parameter -> {
				...
				if (injectedBeanMap.size() > 1) {
					throw new ThriftClientInstantiateException(
							"Detected ambiguous beans of {}" + parameterType.getSimpleName());
				}

				return injectedBeanMap.entrySet().stream().findFirst().map(Map.Entry::getValue)
						.orElseThrow(() -> new ThriftClientInstantiateException(
								"Detected non-qualified bean of {}" + parameterType.getSimpleName()));
			}).toArray();
