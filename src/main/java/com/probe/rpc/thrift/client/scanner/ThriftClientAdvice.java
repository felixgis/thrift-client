package com.probe.rpc.thrift.client.scanner;

import com.orbitz.consul.Consul;
import com.probe.rpc.thrift.client.cache.ThriftServiceMethodCacheManager;
import com.probe.rpc.thrift.client.common.ThriftClientContext;
import com.probe.rpc.thrift.client.common.ThriftServiceSignature;
...
import com.probe.rpc.thrift.client.exception.ThriftApplicationException;
import com.probe.rpc.thrift.client.exception.ThriftClientException;
import com.probe.rpc.thrift.client.exception.ThriftClientOpenException;
import com.probe.rpc.thrift.client.exception.ThriftClientRegistryException;
import com.probe.rpc.thrift.client.exception.ThriftClientRequestTimeoutException;
import com.probe.rpc.thrift.client.loadbalancer.IRule;
import com.probe.rpc.thrift.client.loadbalancer.RoundRobinRule;
import com.probe.rpc.thrift.client.loadbalancer.ThriftConsulServerListLoadBalancer;
	...
	public Object invoke(MethodInvocation invocation) throws Throwable {
		...
		while (true) {
			if (retryTimes++ > poolProperties.getRetryTimes()) {
				logger.error("All thrift client call failed, method is {}, args is {}, retryTimes: {}",
						invocation.getMethod().getName(), args, retryTimes);
				throw new ThriftClientException(
						"Thrift client call failed, thrift client signature is: " + serviceSignature.marker());
			}
			...
			try {
				...
				Method cachedMethod = ThriftServiceMethodCacheManager.getMethod(client.getClass(),
						invocationMethod.getName(), invocationMethod.getParameterTypes());
				...
				if (undeclaredThrowable instanceof TTransportException) {
					...
					if (realException instanceof SocketTimeoutException) { // 超时,直接抛出异常,不进行重试
						...
						logger.error(
								"Thrift client request timeout, ip is {}, port is {}, timeout is {}, method is {}, args is {}",
					...
					logger.error(
							...
							serverNode.getHost(), serverNode.getPort(), serverNode.getTimeout(), invocation.getMethod(),
							args, retryTimes);
					if (retryTimes >= poolProperties.getRetryTimes()) {
				...
				if (e instanceof ThriftClientOpenException) { // 创建连接失败
					...
					if (realCause instanceof SocketException
							&& realCause.getMessage().contains("Network is unreachable")) {
						throw e;
					}
				} else {
					throw e;
